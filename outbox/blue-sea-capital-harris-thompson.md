# DRAFT — NOT SENT — cold email, review before sending

**Target:** Blue Sea Capital — Harris D. Thompson, Operating Partner
**Prepared by:** Blake (Content Manager persona) for Tonya Seavers Evans, North Star Legacy Holdings
**Source:** memory/projects/pe-firms-tracker.md — Qualification Notes (Lisa), "Blue Sea Capital — re-qualified 2026-08-12, policy question resolved same day"
**Angle used:** Blue Sea already sources digital transformation/CRM-ERP work through outside vendors per portfolio company, with no in-house technical leadership — pitch is "you're already buying this externally, let's talk about how it's evaluated and executed."

**PLACEHOLDERS — fill in before sending:**
- `[HARRIS THOMPSON EMAIL]` — Rex confirmed the name and title but not a direct email address; verify before sending

---

**Subject:** How Blue Sea evaluates the digital work it's already outsourcing

Harris,

Your portfolio companies are getting cloud migration, data analytics, and CRM/ERP work done — but from what I can tell, that work is being sourced vendor by vendor, per company, without a function inside Blue Sea that owns the evaluation of it. That's not a criticism. Most firms your size run it that way. It just means nobody's asking the harder question: is the *organization* actually ready to adopt what the vendor is building, or is the technology getting installed faster than the team can change around it.

That gap — readiness, not tooling — is where I work. I'm Tonya Seavers Evans, and I help leadership teams navigate change; AI and digital transformation are just the highest-stakes version of that work right now, not a separate specialty. I'm Prosci-certified, which means my framework for evaluating and managing that readiness is structured and replicable — not consultant intuition. For a firm managing this across a manufacturing, healthcare, and aerospace & defense portfolio, the failure mode is rarely the vendor selection. It's rollout.

I'd like to compare notes on how Blue Sea currently evaluates and manages that work across the portfolio — not to pitch a specific engagement, just to understand where the friction actually shows up. Please send me a few dates and times that work well on your end, and I'll do my best to make my schedule work around yours.

Worth 20 minutes?

Tonya Seavers Evans
Founder & Principal, North Star Legacy Holdings
tonya@northstarlegacyholdings.com
954-854-7511
linkedin.com/in/tonyaseaversevans
